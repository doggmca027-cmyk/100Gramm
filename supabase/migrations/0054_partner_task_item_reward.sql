-- Lets "Смотрящие" (ambassador) and "Связи на районе" (partner) tasks pay
-- out an existing boost item instead of GRAM, same catalog system_tasks
-- already draws from (combo_item_templates/user_inventory, see
-- 0028_system_tasks.sql). A task keeps exactly one reward kind — GRAM
-- (reward_amount, unchanged) or an item (new reward_item_type/qty) — picked
-- by the admin at creation time, not both; the check constraint enforces
-- "some reward" either way.
alter table partner_tasks
  add column reward_item_type text references combo_item_templates(item_type),
  add column reward_item_qty integer not null default 1 check (reward_item_qty > 0);

-- reward_amount was already `not null default ... check (>= 0)` — 0 is a
-- valid value for an item-rewarded task, this just also requires an item
-- when it's 0.
alter table partner_tasks
  add constraint partner_tasks_reward_check
  check (reward_amount > 0 or reward_item_type is not null);

-- ---------------------------------------------------------------------------
-- claim_partner_task — return type changes numeric -> jsonb (grants an item
-- now, not just GRAM), so the old signature has to be dropped first;
-- Postgres won't let CREATE OR REPLACE change a function's return type.
-- Otherwise identical logic/locking to the 0044_partner_task_delayed_reward.sql
-- version, just crediting whichever reward the task actually has.
-- ---------------------------------------------------------------------------
drop function if exists claim_partner_task(uuid, uuid);
...
declare
  ...
  i integer;
begin
  select season_id, reward_amount, reward_item_type, reward_item_qty, is_active
  into v_season_id, v_reward, v_item_type, v_item_qty, v_active
  ...
  return jsonb_build_object(
    ...
    'reward_item_qty', v_item_qty
  );
end;
$$;

-- ---------------------------------------------------------------------------
-- get_player_state — partner_tasks sub-block gains reward_item_type/qty so
-- the client can render an item icon instead of (or alongside) the GRAM
-- amount. Everything else unchanged from 0047_partner_task_kind.sql.
-- ---------------------------------------------------------------------------
