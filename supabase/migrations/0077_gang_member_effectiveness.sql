-- get_player_state's gang.members rows only ever carried identity + role +
-- joined_at — leaders had no way to tell an active member from a dead
-- account without leaving the app to check gang_bank_transactions by hand.
-- Adds three per-member stats, each derived live from tables that already
-- hold full history, so every existing member's numbers are correct from
-- the moment this ships -- there's nothing to backfill:
--   * completed_cycles  -- user_seasons.completed_cycles_total (that
--     member's lifetime finished cycles this season, same counter the
--     player's own wallet.completed_cycles_total already reads).
--   * donated_gram      -- sum of gang_bank_transactions.amount_gram this
--     member has ever put into THIS gang's bank (same rows bank_top_donors
--     / bank_transactions already aggregate, just per-member and
--     unbounded instead of top-N).
--   * last_active_at    -- greatest(last cycle start, last bank donation)
--     for that member; null only for a member who has done neither since
--     joining -- the clearest "never actually played" signal available
--     without adding new tracking. greatest() ignores nulls on either
--     side (returns null only when both are), per Postgres docs.
-- Byte-for-byte identical to 0076's get_player_state otherwise.
