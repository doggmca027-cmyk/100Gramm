-- Splits "Связи на районе" (partner_tasks) into two flavors sharing the same
-- table/verification machinery (title + channel link + reward, verified via
-- getChatMember, same 24h delayed-reward hold from 0044): partner-branded
-- channels ('partner', the existing behavior, default) vs channels run by
-- our own ambassadors ('ambassador', new — "Смотрящие" tab). Nothing about
-- claim_partner_task/record_partner_task_verification/reset_partner_task_verification
-- needs to change — they operate on task_id alone and don't care which kind
-- it is. Only the catalog column, the admin CRUD surface, and get_player_state's
-- projection change.

alter table partner_tasks
  add column kind text not null default 'partner' check (kind in ('partner', 'ambassador'));

-- ---------------------------------------------------------------------------
-- get_player_state — partner_tasks sub-block gains `kind` so the client can
-- split one flat list into the "Смотрящие" and "Связи на районе" tabs.
-- Everything else in this function is unchanged from 0045_system_task_delayed_reward.sql.
-- ---------------------------------------------------------------------------
