-- get_player_state's gang.bank_top_donors / gang.bank_transactions were
-- capped at 5 / 10 rows (0071_open_battle_boosts_to_members.sql). The
-- treasury tab's "see all" modal (client-side only, no new endpoint) wants
-- more to actually show, so both caps rise to 20. Byte-for-byte identical
-- to 0071's get_player_state otherwise.
