-- 100ГРАМ: dynamic GRAM/USDT exchange rate + direct USDT (Jetton) payments.
--
-- GRAM *is* TON (see 0024_ton_deposit_1to1.sql) — there's no independent
-- GRAM/USDT market to quote. So "GRAM/USDT rate" is really just "TON/USDT
-- rate", fetched live from TonAPI's /v2/rates endpoint (lib/gram-rate.ts)
-- and cached here for up to 60s so the UI never blocks on an outbound HTTP
-- call. This table only ever holds the single 'GRAM_USDT' row today, but is
-- keyed by `pair` in case a second quote currency shows up later.
...
insert into exchange_rates (pair, rate, source) values ('GRAM_USDT', 1.4, 'seed');

-- ---------------------------------------------------------------------------
-- set_exchange_rate — upsert used by the app's own rate-refresh code
-- (lib/gram-rate.ts, called opportunistically from /api/state and
-- /api/exchange-rate, plus the /api/cron/refresh-rate backstop). Not meant
-- to be called with attacker-controlled input — the API layer is the one
-- deciding p_rate (from TonAPI), never a raw client body.
-- ---------------------------------------------------------------------------
create or replace function set_exchange_rate(p_pair text, p_rate numeric, p_source text)
returns jsonb
language plpgsql
security definer
...
as $$
begin
  if p_pair is null or length(p_pair) = 0 then
  ...
  values (p_pair, p_rate, coalesce(p_source, 'unknown'), now())
  on conflict (pair) do update
    set rate = excluded.rate, source = excluded.source, updated_at = excluded.updated_at;

  return jsonb_build_object('pair', p_pair, 'rate', p_rate, 'source', p_source);
end;
$$;

-- ---------------------------------------------------------------------------
-- credit_usdt_payment — the USDT/Jetton counterpart of credit_ton_deposit
-- (0024_ton_deposit_1to1.sql). Same shape and the same trust boundary: the
-- caller (API route) has already independently confirmed the on-chain
-- Jetton transfer via TonAPI before this ever runs — p_gram_amount is
-- server-computed from the *actually received* USDT amount divided by the
-- rate at verification time (not a client-claimed quote), same "never trust
-- the client's amount past the poll's matching floor" rule the TON path
-- uses. Reuses wallet_transactions with type = 'purchase' — the ledger
-- doesn't distinguish the settlement currency (TON vs USDT) since neither
-- history nor the admin UI render it, only tx_hash + the credited GRAM.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- get_player_state — adds top-level `exchange_rate`. Whatever's cached in
-- exchange_rates at read time (the API layer is responsible for keeping it
-- ≤60s stale, see lib/gram-rate.ts) — this function only ever reads, never
-- makes the outbound HTTP call itself (plpgsql can't do that reliably,
-- same reasoning as partner_tasks channel verification). Identical to the
-- 0030_daily_quests_drop_gram_reward.sql version otherwise.
-- ---------------------------------------------------------------------------
