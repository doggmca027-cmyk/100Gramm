-- Same treatment as 0044_partner_task_delayed_reward.sql, applied to the
-- OTHER subscription checker in this codebase: system_tasks' social
-- (channel_sub/chat_join) tasks in "Задания района". This is the one
-- actually live for players right now (partner_tasks currently ships with
-- every row is_active=false) — see api/system-tasks/claim/route.ts, which
-- used to call getChatMember and claim_system_task back-to-back in one
-- request, same subscribe->claim->unsubscribe loophole.
--
-- referral/gameplay tasks (and the post_view self-report social task) are
-- entirely unaffected — they never had an external verification step to
-- delay, and keep claiming instantly.
--
-- user_completed_tasks is shared across ALL task categories (unlike
-- partner_tasks, which has its own dedicated table), so this migration is
-- more surgical than 0044: only channel_sub/chat_join rows ever get a
-- verified_at-without-completed_at "pending" state; every other category
-- still inserts with completed_at = now() in the same statement it always
-- did.

alter table user_completed_tasks
  add column verified_at timestamptz;

-- Existing rows all came from the old atomic flow — backfill so the
-- (currently unused, since this table had zero social completions before
-- today per the pre-migration check) history stays consistent.
update user_completed_tasks set verified_at = completed_at where verified_at is null;
...
-- ---------------------------------------------------------------------------
-- record_system_task_verification — mirrors record_partner_task_verification.
-- Called once getChatMember passes for a channel_sub/chat_join task; upserts
-- verified_at = now() the first time, no-ops (keeps the original
-- verified_at) on a repeat call while still pending, raises if already paid.
-- ---------------------------------------------------------------------------
create or replace function record_system_task_verification(p_user_id uuid, p_task_id uuid)
returns jsonb
language plpgsql
security definer
...
declare
  v_season_id uuid;
  v_is_active boolean;
  v_row user_completed_tasks%rowtype;
begin
  ...
  select is_active into v_is_active from system_tasks where id = p_task_id;
  if v_is_active is null or not v_is_active then
    raise exception 'unknown_task';
  end if;

  insert into user_completed_tasks (user_id, season_id, task_id, verified_at)
  values (p_user_id, v_season_id, p_task_id, now())
  on conflict (user_id, season_id, task_id) do nothing;

  select * into v_row from user_completed_tasks
  where user_id = p_user_id and season_id = v_season_id and task_id = p_task_id;
...
-- ---------------------------------------------------------------------------
-- reset_system_task_verification — the 24h-later re-check in
-- api/system-tasks/claim/route.ts calls this when getChatMember no longer
-- passes, wiping the pending row so a future resubscribe restarts the wait.
-- Scoped to the active season (task_id alone doesn't disambiguate season —
-- system_tasks has no season_id of its own, unlike partner_tasks).
-- ---------------------------------------------------------------------------
create or replace function reset_system_task_verification(p_user_id uuid, p_task_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  v_season_id := active_season_id();
  delete from user_completed_tasks
  where user_id = p_user_id and season_id = v_season_id and task_id = p_task_id
...
-- ---------------------------------------------------------------------------
-- claim_system_task — channel_sub/chat_join now require an existing
-- verification at least 24h old and unpaid (same `for update` +
-- completed_at-is-null pattern as claim_partner_task, preventing a double
-- claim race the same way). Every other category is untouched: same
-- progress checks, same immediate insert-with-completed_at.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.claim_system_task(p_user_id uuid, p_slug text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  v_delayed_social := v_task.category = 'social' and v_task.target_type in ('channel_sub', 'chat_join');
  ...
  -- referral/gameplay: progress just checked above. post_view: nothing
  -- Telegram exposes to verify "viewed a post" with, self-report by design.
  -- channel_sub/chat_join: NOT checked here at all anymore — the route
  -- verifies Telegram membership *before* ever calling this function, both
  -- for the initial record_system_task_verification and for this delayed
  -- claim (see api/system-tasks/claim/route.ts). This function only
  -- enforces that a verification exists and is old enough.

  if v_delayed_social then
    ...
    update user_completed_tasks set completed_at = now()
    where user_id = p_user_id and season_id = v_season_id and task_id = v_task.id;
  else
    insert into user_completed_tasks (user_id, season_id, task_id, completed_at)
    values (p_user_id, v_season_id, v_task.id, now());
  end if;
...
-- ---------------------------------------------------------------------------
-- get_player_state — system_tasks sub-block: 'completed' now means "reward
-- paid" instead of "row exists"; adds verified_at/available_at so the
-- client can render the 24h wait and gate the check button. (partner_tasks
-- sub-block already carries this shape from 0044 — untouched here.)
-- ---------------------------------------------------------------------------
