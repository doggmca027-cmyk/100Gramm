-- 100ГРАМ: lets a player (in practice: an admin, via a toggle in the admin
-- panel) hide their own profile from the leaderboard. Purely a display
-- flag — doesn't touch balance/rank/anything else, just excludes the row
-- from get_leaderboard's result and is otherwise invisible everywhere
-- else (the player's own state, history, etc. are all unaffected).
alter table users add column hide_from_leaderboard boolean not null default false;

-- ---------------------------------------------------------------------------
-- get_leaderboard — same as before, now excludes hidden profiles.
-- ---------------------------------------------------------------------------
...
begin
  ...
  from (
    ...
    where us.season_id = v_season_id and not u.hide_from_leaderboard
    order by
...
-- ---------------------------------------------------------------------------
-- get_player_state — same as before, profile now carries the caller's own
-- hide_from_leaderboard flag so the admin panel can show its current state.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- set_leaderboard_visibility — the toggle itself. Always acts on the caller's
-- own account (p_user_id comes from the session-verified admin, never a
-- client-supplied target) — this hides *your own* profile, not anyone else's.
-- ---------------------------------------------------------------------------
